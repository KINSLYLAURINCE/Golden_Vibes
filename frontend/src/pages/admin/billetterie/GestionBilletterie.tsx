...
import { 
  Plus, Edit, Trash2, X, Save, Package, ShoppingCart, Loader2, Upload, Mail, Phone, QrCode, Calendar,
      ...
      {!loadingPacks && packs.length > 0 && (
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          >
            ...
            <p className="text-2xl font-bold text-amber-400">{totalVendus}<span className="text-sm text-muted-foreground">/{totalPlaces}</span></p>
          </motion.div>
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            ...
            className="bg-card border border-border rounded-xl p-4"
          >
            <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">Taux remplissage</p>
            <p className="text-2xl font-bold text-amber-400">{totalPlaces > 0 ? Math.round((totalVendus / totalPlaces) * 100) : 0}%</p>
          </motion.div>
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            <p className="text-2xl font-bold text-amber-400">{totalRevenu.toLocaleString()} <span className="text-sm">FCFA</span></p>
          </motion.div>
        </div>
      )}
              ...
              <table className="w-full min-w-[1100px]">
                <thead>
                  <tr className="border-b border-border bg-secondary/50">
                    {["Réf.", "Date", "Client", "Contact", "Pack", "Qté", "Montant", "Mode", "QR Code", "Paiement", "Billet"].map((h, i) => (
                  ...
                  {billetsFiltres.map((v, index) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-3 whitespace-nowrap">
                        <motion.span 
                          animate={v.statut_paiement === "valide" ? { 
                            boxShadow: ["0 0 0 0 rgba(245,158,11,0.4)", "0 0 0 10px rgba(245,158,11,0)"]
                          } : {}}
                          transition={{ duration: 2, repeat: Infinity }}
            ...
            >
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between py-2 border-b border-border">
                  ...
                  <span className="font-mono text-amber-400 text-xs">{selectedBillet.transaction_id}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-border">
                  <span className="text-muted-foreground">QR Code</span>
                  <span className="font-mono text-xs text-foreground">{selectedBillet.qr_code}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-border">
                  <span className="text-muted-foreground">Client</span>
                  <span className="text-foreground font-medium">{selectedBillet.nom_client}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-border">
                  <span className="text-muted-foreground">Email</span>
                  <span className="text-foreground">{selectedBillet.email}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-border">
                  <span className="text-muted-foreground">Téléphone</span>
                  <span className="text-foreground">{selectedBillet.telephone}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-border">
                  <span className="text-muted-foreground">Pack</span>
                  <span className="text-foreground">{selectedBillet.pack?.nom || "—"}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-border">
                  <span className="text-muted-foreground">Quantité</span>
                  <span className="text-foreground">{selectedBillet.quantite}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-border">
                ...
                <div className="flex justify-between py-2 border-b border-border">
                  ...
                  <span className="text-foreground capitalize">{selectedBillet.mode_paiement}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-border">
                  <span className="text-muted-foreground">Statut paiement</span>
                  <motion.span 
                    animate={selectedBillet.statut_paiement === "valide" ? { 
                      boxShadow: ["0 0 0 0 rgba(245,158,11,0.4)", "0 0 0 10px rgba(245,158,11,0)"]
                    } : {}}
                    transition={{ duration: 2, repeat: Infinity }}
                ...
                <div className="flex justify-between py-2">
                  ...
                  <span className="text-foreground text-xs">{selectedBillet.created_at?.slice(0, 16).replace("T", " ")}</span>
                </div>
              </div>
        ...
        {showForm && (
          <div className="fixed inset-0 bg-background/80 z-50 flex items-center justify-center p-4">
            <motion.form onSubmit={sauvegarder}
                ...
                <label className={`flex flex-col items-center justify-center h-36 border-2 border-dashed rounded-lg cursor-pointer transition-all bg-secondary overflow-hidden ${imagePreview || existingImage ? "border-amber-500 border-solid" : "border-border hover:border-amber-500/50"}`}>
                  {imagePreview ? (
                    ...
                    <div className="relative w-full h-full">
                      <img src={getImageUrl(existingImage)} alt="Actuelle" className="h-full w-full object-cover" onError={e => { e.target.style.display = "none"; }} />
                      <span className="absolute bottom-1 left-1/2 -translate-x-1/2 text-xs text-white bg-black/60 px-2 py-0.5 rounded whitespace-nowrap">Cliquer pour remplacer</span>
                    </div>
                  ) : (
              ...
              <div>
                <label className="block text-sm text-muted-foreground mb-1">Avantages <span className="text-xs text-muted-foreground/60">(un par ligne)</span></label>
                <textarea name="avantages" value={form.avantages} onChange={handleChange} rows={4} placeholder={"Accès VIP\nBoisson offerte\nPhoto souvenir"}
              ...
              <div className="flex gap-3 pt-2">
                ...
                <motion.button
                  ...
                  {saving ? <><Loader2 size={16} className="animate-spin" /> En cours...</> : <><Save size={16} /> {editId ? "Sauvegarder" : "Créer"}</>}
                </motion.button>
              </div>
            </motion.form>
          </div>
